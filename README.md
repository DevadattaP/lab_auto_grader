...
- Any files placed in `questions/<lab_id>/shared_files/` (question paper PDF, images, datasets, any file type — a flat folder, no subfolders) appear in a "Shared files" dropdown, each a direct download link. Available while the session is running, and again once it's finalized (so students can keep referring to them alongside their report/leaderboard). Hidden before the session starts, while merely locked (mid-grading, not yet finalized), or if the admin turns it off (Session tab). The rest of `questions/<lab_id>/` — gold solutions, hidden test cases — is never exposed this way.
...
| **Shared files** | Drop any files (question paper PDF, images, datasets, ...) into `questions/<lab_id>/shared_files/` and students see a download menu for them (auth-gated, not static assets) — toggleable by the admin, and automatically hidden before start / while locked / and shown again once finalized. |
...
└── display_config.json         # Toggles (show_workspace_after_session, show_report, show_leaderboard, show_shared_files) — runtime-editable by admin

questions/lab_01/
└── shared_files/                # Optional — any files here (question paper, images, datasets, ...) are downloadable by students while the session is running/finalized; never the rest of questions/lab_01/ itself
