...
- **Hybrid**: can confuse `isolate`'s auto-detection. Fix by forcing full v2 — add `systemd.unified_cgroup_hierarchy=1` to the GRUB kernel command line, `update-grub`, reboot. One-time machine prep, not something fixable at grading time.
