...
## 22. Question paper download, bulk account actions, and roster-restricted login

- **Question paper download**: each lab can have a `questions/<lab_id>/<lab_id>_qp.pdf` (the full, human-authored question paper, distinct from the structured per-question configs used for grading). A new `GET /api/question-paper` route on `server_student` serves it via `send_file`. Deliberately **not** placed under `server_student/static/` — that folder is served unauthenticated by Flask, and this needed the same session + timing gates as every other student endpoint. It's also deliberately *different* from the existing `_workspace_gate_error`: available while `live_session` status is `running` **or** `finalized` (per explicit follow-up ask, so students can keep referring to the paper alongside their published report/leaderboard, matching how workspace browsing already stays open then), but never while merely `locked` (mid-grading, not yet finalized) or before the session has started — gated by a dedicated `_question_paper_available(state)` helper shared between `/api/session/status` (drives the button's visibility, polled every heartbeat) and `/api/question-paper` itself (so hitting the URL directly can't bypass what the button already hides). `DisplayConfig` gained a fourth independently-toggleable flag, `show_question_paper` (default on), following the exact same runtime-editable pattern as the other three (`grader/display_config.py`, read fresh by `server_student` on every relevant request, written by a new checkbox in `server_admin`'s Session tab under its own "While the session is live, students can..." heading) — so an admin can turn off paper access mid-session without restarting anything, on top of the automatic running-only gate. The student-side button starts `hidden` and is shown/hidden by `pollStatus()` alongside the timer/lock/workspace logic, not by the one-shot `/api/config` fetch, since it needs to react within one heartbeat to both the session status changing and the admin's toggle changing.
- **Mass account actions**: the Accounts tab gained a checkbox column (plus a header select-all) and a bulk-action toolbar that appears only when ≥1 row is selected, with "Sign out device" and a single combined lock/unlock button. Per explicit spec, the lock/unlock button's label and enabled state are derived from the selection's current lock states, not just "the first selected row": **"Lock"** (enabled) only when every selected account is currently unlocked, **"Unlock"** (enabled) only when every selected account is currently locked, and disabled for any mixed selection — the disabled state blocks the click handler itself, not just the button's appearance, so a mixed selection can't fire the bulk API by any path. No new backend endpoints: both actions loop the pre-existing single-account `/unbind`, `/lock`, `/unlock` routes per selected roll number, with exactly one `confirm()` before the loop starts and no per-iteration alert (unlike the single-row buttons, which alert after every individual action) — a deliberate UX choice for the bulk path specifically.
- **Real CSS bug, same root cause as §20's item (2), found again**: the new bulk toolbar (`class="form-row" hidden`) stayed visibly rendered with stale content even at zero selected, because `.form-row { display: flex }` is an author-origin rule and always beats the browser's default `[hidden] { display: none }`, regardless of specificity — this codebase had already hit and fixed the identical bug once for view containers (§20), but the shared `.form-row` class hadn't been covered. Fixed the same way: an explicit `.form-row[hidden] { display: none }` override.
- **Roster-restricted login**: a fifth `DisplayConfig` toggle, `restrict_login_to_roster` (default **off**, unlike the other four — turning it on with no roster CSV loaded would lock out every student, so it can't default to a safe-sounding "on"). When enabled, `server_student`'s `POST /api/login` rejects any roll number not present in `STUDENT_MAPPING.entries` (the same optional roll_no→name mapping already loaded from `--student-names-csv`/`$STUDENT_NAMES_CSV`/`live/student_names.csv` for display purposes) with the same generic "Incorrect roll number or password" used for a genuinely wrong attempt, so an unlisted roll number can't be distinguished from a typo by response shape. Checked *before* calling `accounts.authenticate_student`, not after — that call auto-provisions a brand-new global account the moment a correct default-password attempt is seen, so checking membership afterwards would let an unlisted student's very first attempt silently create an account moments before being rejected. The checkbox lives on `server_admin`'s Accounts tab (not Session, per explicit spec — this is an account/login policy, not a display toggle) and saves immediately on change rather than waiting for the batched display-config "Save" button; the admin GET response also returns `roster_size` (`len(STUDENT_MAPPING.entries)` on the admin side) purely so the UI can refuse to enable the toggle client-side with an empty/no roster loaded, rather than let an admin lock out the whole class by accident. Deliberately login-time-only, not a live re-check on every request like `_current_roll_no`'s active/IP check: flipping this on does not retroactively kick out students who are already logged in and not on the list, only blocks new login attempts going forward — an explicit scope decision, not an oversight.

## 23. Question paper generalized to a shared-files folder; Finalize button race fixed

- **Single question-paper PDF generalized to a shared-files folder**: `questions/<lab_id>/shared_files/` (a fixed, flat folder -- no subfolders, no recursion) replaces the old single `questions/<lab_id>/<lab_id>_qp.pdf` convention from §22. An admin can drop any number of files of any type here (question paper PDF, images, datasets, ...); students see them as a "Shared files" dropdown menu of direct download links rather than a single button. Deliberately still never the rest of `questions/<lab_id>/` (gold solutions, hidden test cases) -- `_shared_files_dir()` is a hardcoded child path, not admin-configurable, so there's no way to accidentally point it at grading material. `_question_paper_available`/`_question_paper_path` were renamed to `_shared_files_available`/`_shared_files_dir`/`_list_shared_files`; `DisplayConfig.show_question_paper` renamed to `show_shared_files` (same running-or-finalized-but-not-locked gate as before, per §22). Renaming an on-disk `display_config.json` field is a real migration hazard -- `get_config`/`set_config` previously did `DisplayConfig(**json.loads(...))` directly, which would `TypeError` on any lab's config file still carrying the old `show_question_paper` key; fixed with a `_load()` helper that filters the parsed JSON down to `{f.name for f in fields(DisplayConfig)}` before constructing, so an old key is silently dropped (the renamed toggle reverts to its new default) instead of bricking that lab's config reads. `GET /api/question-paper` became `GET /api/shared-files/<filename>`, which re-validates `filename` against a fresh `_list_shared_files()` call (not just "no slashes") before serving, so a filename can't be probed/guessed for something that was never actually placed in the folder — verified against literal and URL-encoded (`..%2Fapp.py`, `%2e%2e%2f...`) traversal attempts, both correctly 404 rather than escaping the folder. `GET /api/session/status` now returns a `shared_files` array (the actual filenames) instead of a boolean, and the client's `renderSharedFiles()` diffs against the last-rendered list (by JSON string) before touching the DOM, so an unrelated heartbeat poll doesn't rebuild the `<ul>` (and close an open `<details>` menu) every cycle. `server_admin` also gained a read-only `GET /api/live/<lab_id>/shared-files` listing endpoint purely so an admin can confirm what's actually in the folder without SSHing into the machine — no upload UI, files are still managed directly on disk.
- **Finalize button race, fixed**: the "Finalize & Grade" button could be clicked again while a grading run was already in progress, because `loadSessionStatus()` (polled every 5s) unconditionally set `finalizeBtn.disabled = data.status !== "locked"` — and the session stays in `locked` status for the *entire* grading run (it only becomes `finalized` once grading actually completes), so that poll was silently re-enabling the button every 5 seconds, overriding the click handler's own `disabled = true`. Fixed with a `finalizeRunning` flag (distinct from `currentStatus`), set the moment a grading run starts (from the click handler, or from `pollFinalizeStatus()` detecting one already in progress on page load) and cleared only when that same poll sees the run reach `done`/`error`; `loadSessionStatus()` now skips touching `finalizeBtn.disabled` entirely while the flag is set, so no code path can re-enable the button mid-run.
